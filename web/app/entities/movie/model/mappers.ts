...
export function mapMovieDetails(movie: MovieDetailsApiResponse): Movie {
  return {
    id: movie.id_kp,
    ...
    age: movie.age_restrictions ? String(movie.age_restrictions) : null,
    ...
    genres: movie.genre.split(",").map((item) => item.trim()),

    actors: movie.actors.split(",").map((item) => item.trim()),

    directors: movie.directors.split(",").map((item) => item.trim()),

    producers: movie.producers.split(",").map((item) => item.trim()),
...
export function mapAllohaMovie(response: AllohaMovieResponse): Movie {
  const movie = response.data;

  if (!movie) {
    throw new Error("Movie details are not available");
  }
  ...
  return {
    ...
    translations: Object.entries(movie.translation_iframe ?? {}).map(
      ([id, translation]) => ({
        id,
